...
class DataProcessor(object):
  ...
  def _read_csv(cls,input_file):
        """Reads a tab separated value file."""
        df = load_csv(input_file,header=0).fillna('|')
        jobcontent = df['review'].tolist()
        jlabel=[[0 for col in range(4)] for row in range(len(jobcontent))]
        jtmp=df['label'].tolist()
        ...
        #print('############test#########')
        #for i in range(len(jlabel)):
        #  print(i)
        #  print(jlabel[i])
        #  print([id2label(l) for l in range(4) if jlabel[i][l]==1])

          #print([id2label(0) for l in np.where(jlabel[i]==1)[0]])
        #  print(jobcontent[i])
        #  print('\n')
        #print(hp.label_vocabulary)
        #print('Read csv finished!(1)')
